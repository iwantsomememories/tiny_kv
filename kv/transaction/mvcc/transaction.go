...
func (txn *MvccTxn) GetValue(key []byte) ([]byte, error) {
	...
	for ; iter.Valid(); iter.Next() {
		...
		if commitTs <= txn.StartTS {
			...
			userVal, err := txn.Reader.GetCF(engine_util.CfDefault, EncodeKey(key, write.StartTS))
			if err != nil {
				return nil, err
			}
			return userVal, nil
...
func (txn *MvccTxn) PutValue(rawKey []byte, value []byte) {
	// Your Code Here (4A).
	txn.writes = append(txn.writes, storage.Modify{Data: storage.Put{
		Key:   EncodeKey(rawKey, txn.StartTS),
		Value: value,
